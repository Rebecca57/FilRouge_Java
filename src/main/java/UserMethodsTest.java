...
public class UserMethodsTest {
	...
	public void testLogin() throws Exception {
		...
		//user.setF
		when(um.login(user)).thenReturn(user);
		//when(ur.login(user)).thenReturn(ResponseEntity.ok().body(user));		

		// WHEN
		final ResponseEntity<User> result = ur.login(user);
				

		// THEN
		verify(um).login(user);
		assertThat(result).isEqualTo(ResponseEntity.ok().body(user));
	}**/
